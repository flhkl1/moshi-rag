...
impl ScatteredCacheBuilder {
    ...
    /// Copy positions and indices from another builder (e.g. when copying depformer slice state).
    pub fn copy_from(&mut self, other: &Self) {
        assert_eq!(self.batch_size(), other.batch_size(), "builder batch_size mismatch");
        assert_eq!(self.context, other.context, "builder context mismatch");
        self.positions.copy_from_slice(&other.positions);
        self.indices.copy_from_slice(&other.indices);
    }
