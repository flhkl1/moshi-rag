...
class LMModel(StreamingContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Special token for RAG trigger signal
    ...
    def forward(self, codes: torch.Tensor, condition_tensors: tp.Optional[ConditionTensors] = None) -> LMOutput:
        ...
        prepend_condition: torch.Tensor | None = None
        ...
        if condition_tensors is None:
            ...
            prepend_condition = self.fuser.get_prepend(condition_tensors)
            streaming_sum_condition = self.fuser.get_streaming_sum(condition_tensors)
        transformer_out, text_logits = self.forward_text(
            sequence=torch.cat([prepend_condition, delayed_codes[:, :, :-1]], dim=1)
            if prepend_condition is not None
            else delayed_codes[:, :, :-1],
            sum_condition=sum_condition,
            cross_attention_src=cross_attention_src,
            streaming_sum_condition=streaming_sum_condition,
        )
    ...
    def forward_text(
        ...
        assert sequence is not None or sequence_emb is not None, "Either sequence or sequence_emb must be provided."
        assert sequence is None or sequence_emb is None, "Only one of sequence or sequence_emb must be provided."
        input_ = None
        if sequence is not None:
            ...
            _, S = sequence_emb.shape[0], sequence_emb.shape[1]
            input_ = sequence_emb
        # Apply per-step streaming sum if provided.
        assert input_ is not None
        if streaming_sum_condition is not None:
            condition_len = streaming_sum_condition.shape[1]
            if condition_len > 0:
...
class _LMGenState(State):
    ...
    condition_prepend: torch.Tensor | None = None
...
class LMGen(StreamingModule[_LMGenState]):
    def __init__(
        ...
        self.needed_tokens = lm_model.num_codebooks - lm_model.dep_q - 1
    ...
    def _init_streaming_state(self, batch_size: int) -> _LMGenState:
        ...
        if self.condition_tensors is None:
            condition_sum = None
            condition_cross = None
            condition_prepend = None
            condition_streaming_sum = None
        else:
            assert self.condition_tensors is not None
            assert self.lm_model.fuser is not None
            ...
            condition_prepend = self.lm_model.fuser.get_prepend(self.condition_tensors)
            ...
            if condition_prepend is not None:
                condition_prepend = condition_prepend.to(self.lm_model.dtype)
            ...
            logger.info(
                f"[LM] fused conditions: sum={'yes' if condition_sum is not None else 'no'}, "
                f"cross={'yes' if condition_cross is not None else 'no'}, "
                f"prepend={tuple(condition_prepend.shape) if condition_prepend is not None else 'no'}, "
                f"streaming_sum={tuple(condition_streaming_sum.shape) if condition_streaming_sum is not None else 'no'}"
            )

        if self.cfg_coef != 1.0:
            batch_size *= 2
            if condition_sum is not None:
                assert condition_sum.shape[0] == batch_size, "cfg requires 2x more conditions."
            if condition_cross is not None:
                assert condition_cross.shape[0] == batch_size, "cfg requires 2x more conditions."
            if condition_streaming_sum is not None:
                assert condition_streaming_sum.shape[0] == batch_size, "cfg requires 2x more conditions."
        exit_stack = self.lm_model.streaming(batch_size)
        exit_stack.__enter__()

        disable = lm_model.device.type != "cuda"
        graphed_main = CUDAGraphed(lm_model.forward_text, disable=disable or self.profile)
        if lm_model.depformer is not None:
            graphed_depth = CUDAGraphed(self.depformer_step, disable=disable or self.profile)
        else:
            graphed_depth = None
        ...
        state = _LMGenState(
            ...
            condition_prepend=condition_prepend,
        ...
        state.exit_stack = ExitStack()
        state.exit_stack.enter_context(exit_stack)

        def _reset_callback(reset_mask: torch.Tensor) -> None:
            if self.cfg_coef != 1.0:
                reset_mask = reset_mask.repeat(2)
            self.lm_model.reset_streaming(reset_mask)
            # Apply prepend for reset slots: temporarily restrict exec mask to
            # just the reset entries, run prepend, then let the caller restore
            # the real exec mask via the subsequent set_exec_mask call.
            if state.condition_prepend is not None and state.condition_prepend.shape[1] > 0:
                self.lm_model.set_exec_mask(reset_mask)
                self.lm_model.forward_text(sequence_emb=state.condition_prepend)
            # Clear pending streaming sums for reset slots.
            for b, reset in enumerate(reset_mask.tolist()):
                if reset:
        ...
        # Apply prepend condition at init time (all slots active).
        if state.condition_prepend is not None and state.condition_prepend.shape[1] > 0:
            logger.info(f"[LM] applying prepend condition at init ({tuple(state.condition_prepend.shape)})")
            self.lm_model.forward_text(sequence_emb=state.condition_prepend)

        return state

    def update_streaming_sum_tensors(self, tensors: list[torch.Tensor | None]):
        """Set pending streaming-sum tensors, one per batch slot.

        Args:
            tensors: list of length ``batch_size``.  Each element is either
                ``None`` (no update for that slot) or a tensor of shape
                ``[T, dim]`` giving the per-step streaming-sum values to apply.
        """
        ...
        for b, t in enumerate(tensors):
            ...
            # None entries are left untouched (caller explicitly passes None to skip).

    def apply_pending_streaming_sum_condition(self, exec_mask: torch.Tensor):
        """Consume one step of pending streaming-sum for each executing slot.

        Args:
            exec_mask: CPU bool tensor of shape ``[batch_size]``.
        """
        state = self._streaming_state
        assert state is not None
        assert state.condition_streaming_sum is not None
    ...
    def _step(
        ...
        assert Ki >= self.needed_tokens, f"We expect {self.needed_tokens} tokens from the user stream, got {Ki}."

        if Ki > self.needed_tokens:
            input_tokens = input_tokens[:, : self.needed_tokens, :]
        ...
        if self.check:
            ...
            assert not (input_ == lm_model.ungenerated_token_id).any(), (input_,)
