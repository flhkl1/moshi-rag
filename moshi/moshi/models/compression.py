...
class MimiModel(CompressionModel[_MimiState]):
    ...
    def _init_streaming_state(self, batch_size: int) -> _MimiState:
        ...
        graphed_tr_enc = None
        if self.encoder_transformer is not None:
            graphed_tr_enc = CUDAGraphed(self.encoder_transformer, disable=disable or self.profile)
        if self.decoder_transformer is not None:
            graphed_tr_dec = CUDAGraphed(self.decoder_transformer, disable=disable or self.profile)
        graphed_encoder = CUDAGraphed(self.encoder, disable=disable or self.profile)
        graphed_decoder = CUDAGraphed(self.decoder, disable=disable or self.profile)
        return _MimiState(batch_size, device, graphed_tr_enc, graphed_tr_dec, graphed_encoder, graphed_decoder)

    def set_profile(self, profile: bool) -> None:
