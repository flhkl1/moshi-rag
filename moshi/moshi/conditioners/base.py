...
class EmbeddedText(tp.NamedTuple):
    embeddings: torch.Tensor  # should be float tensor.
    mask: torch.Tensor
...
class BaseConditioner(nn.Module, tp.Generic[Prepared]):
    ...
    def forward(self, inputs: Prepared) -> ConditionType:
        # Get base condition from subclass
        condition_result = self._get_condition(inputs)

        # Unpack possibly timed result
        cond, mask = condition_result
        ...
        # Apply output projection and padding with dtype consistency
        dtype = cond.dtype
        ...
        for fuse_method, conditions in fuse2cond.items():
            for condition in conditions:
                ...
                if fuse_method not in ["cross", "sum", "prepend", "streaming_sum"]:
                    raise RuntimeError(
                        "only `sum`, `cross`, `prepend` and `streaming_sum` conditionings are supported "
